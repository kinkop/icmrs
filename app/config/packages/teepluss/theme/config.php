	...
	*/
	...
	'events' => array(
		...
		'asset' => function($asset)
		{
            ...
            $validates = \Config::get('validate');

            $script = '
                {
                    \'themeAssetUrl\' : \'' . $asset->url('') . '\',
                    \'redirectTime\' : ' . \Config::get('misc.redirect_time') . ',
                    \'validate_rules\' : ' . json_encode($validates) . '
                };
            ';
            ...
            $asset->writeScript('inline-script', '
                 var GlobalData=' . $script . '
            ', $dependencies);

            $asset->container('footer')->usePath()->add('plugin-jquery-form-script', 'js/plugins/jquery.form.min.js');
            $asset->container('footer')->usePath()->add('custom-lib-script', 'js/custom/lib.js');
            $asset->container('footer')->usePath()->add('custom-main-script', 'js/custom/main.js');
